    ...
    getCurrentCoords()
      .then((coords) => {
        generateQr.mutate(
          ...
          {
            onSuccess: async (res) => {
              setQrDataUrl(await QRCode.toDataURL(res.payload, { margin: 1, width: 220, color: { dark: '#16223b' } }));
            },
            onError: (err: any) => {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[calc(100%-1.5rem)] max-w-[400px] p-0 gap-0 overflow-hidden">
              ...
              <div className={`w-full flex flex-col gap-2 p-3 rounded-[var(--radius-md)] border ${isLight ? 'bg-[#efe7d8] border-[#dcf0e4]' : 'bg-[#3f7a5c]/30 border-[#3f7a5c]/40'}`}>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex-1 py-2 bg-gradient-to-r from-[#3f7a5c] to-[#7fbfa0] text-white font-label-caps text-[10px] font-bold uppercase rounded-xl"
                  >
                    Sí, nos vimos
                  </Button>
                  <Button
        ...
        {verificationSuccess && (
          ...
          >
            <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-[#3f7a5c] to-[#7fbfa0] flex items-center justify-center text-white shadow-xl shadow-[#3f7a5c]/30">
              <span className="material-symbols-outlined text-[36px]">verified</span>
            </div>
            <div>
              <span className="font-label-caps text-[9px] uppercase tracking-widest text-[#3f7a5c] font-bold block">
